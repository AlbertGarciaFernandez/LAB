  ...
  return (
    <div className="space-y-4">
      {modules.map((module) => {
        ...
        return (
          <LabCard key={module.slug} className="bg-[linear-gradient(180deg,#ffffff_0%,#fbf8f2_100%)]">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
              <div className="max-w-2xl">
                <div className="flex flex-wrap items-center gap-3">
                  <h3 className="text-xl font-semibold tracking-[-0.03em] text-black">{module.title}</h3>
                  <ProgressBadge value={module.progressPercent} label="mapped" />
                </div>
                <p className="mt-3 text-sm leading-6 text-black/62">{module.summary}</p>
                <p className="mt-4 text-xs font-semibold uppercase tracking-[0.22em] text-black/38">
