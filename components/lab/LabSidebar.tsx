...
export default function LabSidebar({ locale }: LabSidebarProps) {
  ...
  return (
    <aside className="border-black/8 flex h-full flex-col rounded-[30px] border bg-[linear-gradient(180deg,#fffdf8_0%,#f6f1e7_100%)] p-5 shadow-[0_24px_80px_rgba(17,24,39,0.08)]">
      <Link href={`/${locale}/lab`} className="flex items-center gap-3 text-black">
        ...
        <div>
          ...
          <p className="text-sm font-semibold tracking-[-0.02em] text-black">
            {data.landing.brandName}
          </p>
        </div>
      </Link>

      <div className="border-black/8 mt-8 rounded-[24px] border bg-white/80 p-4">
        ...
        <p className="text-black/58 mt-1 text-sm">{data.user.role}</p>
          ...
          <div className="mt-3 space-y-2">
            {secondaryItems.map((item) => (
              <Link
                ...
                className="border-black/8 block rounded-[22px] border bg-white/75 px-4 py-3 text-sm font-medium text-black transition-colors hover:bg-white"
              >
                {item.label}
              </Link>
            ))}
