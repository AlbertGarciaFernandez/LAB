  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        "@type": "ListItem",
        position: 3,
        name: "n8n Migration Consulting",
        item: `https://www.codehunterlab.com/en/expertise/n8n-migration-consulting`,
      },
    ],
  };
          ...
          <div className="mx-auto max-w-7xl px-6">
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {[
                ...
                const col = t.raw(`Comparison.${key}`) as {
                  name: string;
                  cons?: string[];
                  pros?: string[];
                };
                return (
                  <GlassCard
                    key={key}
                    glowColor={negative ? undefined : "orange"}
                    className={`p-8 ${!negative ? "border-hunter-orange/30" : ""}`}
                  >
                    <h3
                      className={`mb-6 text-xl font-bold ${!negative ? "text-hunter-orange" : "text-gray-400"}`}
                    >
                      {col.name}
                    </h3>
                    <ul className="space-y-3">
                      {(col.cons || col.pros || []).map((item: string) => (
                        <li key={item} className="flex items-start gap-3 text-sm text-gray-300">
                          <span
                            className={`mt-0.5 shrink-0 ${negative ? "text-red-400" : "text-hunter-green"}`}
                          >
                            {negative ? "✕" : "✓"}
                          </span>
                          {item}
