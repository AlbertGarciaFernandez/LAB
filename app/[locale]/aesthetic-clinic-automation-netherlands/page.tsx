...
import { CheckIcon, LightningIcon, CalendarXIcon, ChatCircleDotsIcon, ChartBarIcon, ArrowsClockwiseIcon, UsersIcon, CalendarCheckIcon, EnvelopeIcon, RobotIcon } from "@phosphor-icons/react/dist/ssr";

const painPointIcons = [LightningIcon, CalendarXIcon, ChatCircleDotsIcon, ChartBarIcon, ArrowsClockwiseIcon, UsersIcon];
const solutionIcons = [LightningIcon, CalendarCheckIcon, EnvelopeIcon, ChartBarIcon, ArrowsClockwiseIcon, RobotIcon];
...
const aestheticSystems = [
    "WhatsApp Business API", "Instagram API", "n8n", "HubSpot", "Google Ads",
    "Meta Ads", "Planfy", "Timely", "Jane App", "ActiveCampaign", "Mollie",
];

export default function AestheticClinicAutomation({ params: { locale } }: { params: { locale: string } }) {
    ...
    const serviceJsonLd = {
        ...
        "description": "Custom CRM, lead nurturing, and booking automation systems for aesthetic, cosmetic, and dermatology clinics in the Netherlands — instant lead response, post-consult nurture sequences, full funnel attribution, and patient retention automation.",
    ...
    const breadcrumbJsonLd = {
        ...
        "itemListElement": [
            ...
            { "@type": "ListItem", "position": 2, "name": "Aesthetic Clinic Automation Netherlands", "item": `${baseUrl}/${locale}${path}` },
        ],
    };
        ...
        <main className="relative min-h-screen bg-near-black text-white overflow-hidden">
            ...
            <section className="py-12 border-y border-white/5 bg-near-black/50 backdrop-blur-sm relative z-10 overflow-hidden mt-8">
                <div className="flex space-x-12 animate-marquee whitespace-nowrap opacity-40 grayscale hover:grayscale-0 transition-all duration-700">
                    {[...aestheticSystems, ...aestheticSystems].map((sys, i) => (
                        <span key={i} className="text-xl md:text-2xl font-black tracking-tighter text-white uppercase italic">{sys}</span>
                    ))}
                </div>
            </section>
            ...
            <section className="py-32 relative z-10">
                <div className="max-w-5xl mx-auto px-6">
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        {(t.raw("Scenarios.items") as Array<{ num: string; title: string; desc: string }>).map((item, idx) => (
                            <GlassCard key={idx} className="p-8 border-l-4 border-l-hunter-green group hover:bg-white/[0.02] transition-colors" hoverEffect={false}>
